...
1. `${CLAUDE_PLUGIN_ROOT}/bin/kb list` *(via kb:info)* — discover which KBs exist.
2. `${CLAUDE_PLUGIN_ROOT}/bin/kb brief <kb>` *(via kb:info)* — read scope before searching.
3. `${CLAUDE_PLUGIN_ROOT}/bin/kb pending [<kb>]` — what's unprocessed in the inbox (useful when picking up a session).
4. `${CLAUDE_PLUGIN_ROOT}/bin/kb recall [<kb>] --query "<text>"` — search synthesised material in indexable sections.
5. `${CLAUDE_PLUGIN_ROOT}/bin/kb recall [<kb>] --tag <tag>` — list pages carrying a tag.
6. `${CLAUDE_PLUGIN_ROOT}/bin/kb search <kb> "<query>"` — widen to include raw `inbox/`.
7. `${CLAUDE_PLUGIN_ROOT}/bin/kb open <kb> <path>` — read a specific file once you know the path.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/kb recall [<kb>] --query "<text>" [--max-results N]
${CLAUDE_PLUGIN_ROOT}/bin/kb recall [<kb>] --tag <tag>
${CLAUDE_PLUGIN_ROOT}/bin/kb pending [<kb>]                    # unprocessed inbox material
${CLAUDE_PLUGIN_ROOT}/bin/kb search <kb> "<query>"             # wider net (includes inbox)
${CLAUDE_PLUGIN_ROOT}/bin/kb open <kb> <relative/path>
```
