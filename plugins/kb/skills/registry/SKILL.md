...
| Command | Wraps |
|---|---|
| `/kb:bootstrap <name> [--path ...] [--remote ...]` | `${CLAUDE_PLUGIN_ROOT}/bin/kb bootstrap` |
...
`${CLAUDE_PLUGIN_ROOT}/bin/kb remove` is not surfaced as a slash command — it's mutating and should remain explicit-flag-only via the CLI.

## CLI commands

```bash
${CLAUDE_PLUGIN_ROOT}/bin/kb bootstrap <name> [--path <path>] [--remote <url>]   # create or clone a KB
${CLAUDE_PLUGIN_ROOT}/bin/kb add <name> --path <path> [--remote <url>]           # register an existing KB
${CLAUDE_PLUGIN_ROOT}/bin/kb remove <name> [--delete-local --yes]                # remove from registry (files preserved by default)
${CLAUDE_PLUGIN_ROOT}/bin/kb sync <name> | ${CLAUDE_PLUGIN_ROOT}/bin/kb sync --all                         # pull-rebase + push for the KB's remote
${CLAUDE_PLUGIN_ROOT}/bin/kb status [<kb>] | ${CLAUDE_PLUGIN_ROOT}/bin/kb status --all                     # path / git / contract state
```
...
- `status` is read-only; safe to run any time. Bare `${CLAUDE_PLUGIN_ROOT}/bin/kb status` shows all KBs.
