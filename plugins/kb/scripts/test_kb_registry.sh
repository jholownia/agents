...
# T12 — Plugin does not gitignore the distill ledger. After `kb distill
# record`, no `.kb-internal/.gitignore` exists and the ledger is visible to
# git so `kb-dream` commits it during apply.
GI_KB=$TMPDIR/gi-kb
...
if [ "$LEDGER_VISIBLE" -ge 1 ]; then
    PASS=$((PASS+1)); echo "  PASS  T12 findings.ndjson visible to git"
else
