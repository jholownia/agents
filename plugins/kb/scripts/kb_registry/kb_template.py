...
default doesn't fit this KB's domain, change it here and proceed; git
...
- Preserve consumed inbox/source paths in the canonical page. `git log`
  and `inbox/processed/` are the audit trail — don't hand-duplicate it.
- `LOG.md` is bounded, not a journal: each pass **overwrites** its
  `## Latest consolidation` section and **edits** `## Open follow-ups`
  (add new, remove resolved). Full history is `git log`.
...
- `LOG.md` - latest consolidation + open follow-ups (full history in git)
...
**Full history lives in git** — this file is not an append-only journal.
`git log --oneline -- LOG.md` lists every consolidation pass; `git show <hash>`
shows what each one changed. To keep the read cost bounded (the consolidation
agent reads this file every pass), `LOG.md` holds only two living, bounded
sections:

1. **Open follow-ups** — unresolved questions and action items, edited in place:
   add when surfaced, **remove when resolved**.
2. **Latest consolidation** — a snapshot of the most recent pass for continuity.
   **Overwritten each pass, not appended.**

Single sources of truth (do not duplicate here):

- What's been processed -> `inbox/processed/` + `git log`.
- What's been surfaced (distill findings) -> `kb distill surface <kb>`.
...
_No consolidation passes yet._
"""
